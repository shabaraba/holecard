...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Add { name, field } => handlers::vault::handle_add(name, field, &keyring, &config_dir),
        ...
        Commands::Run { entry, command } => handlers::template::handle_run(&entry, &command, &keyring, &config_dir),
        Commands::Lock => handlers::session::handle_lock(&config_dir),
        Commands::Status => handlers::session::handle_status(&config_dir),
        Commands::Export { file } => handlers::transfer::handle_export(&file, &keyring, &config_dir),
        Commands::Import { file, overwrite } => {
            handlers::transfer::handle_import(&file, overwrite, &keyring, &config_dir)
        }
        Commands::Totp { subcommand } => handlers::totp::handle_totp(subcommand, &keyring, &config_dir),
    }
}
